import { DownloadStatus, MediaType } from '../types/index.js';
export class AdvancedUIManager {
    constructor(debugConfig, uiConfig) {
        this.container = null;
        this.shadowRoot = null;
        this.isVisible = false;
        this.currentSources = [];
        this.selections = new Map();
        this.downloadProgresses = new Map();
    ...
    setupEventListeners() {
        ...
        document.addEventListener('click', (e) => {
            if (this.isVisible && this.uiConfig.autoHide && !this.container?.contains(e.target)) {
                this.hide();
            }
        });
    }
    ...
    `;
        const checkbox = item.querySelector(`#${sourceId}`);
        checkbox?.addEventListener('change', () => {
            this.updateSelection(sourceId, source, checkbox.checked);
        });
        const qualitySelector = item.querySelector(`#${sourceId}_quality`);
        qualitySelector?.addEventListener('change', () => {
            this.updateQualitySelection(sourceId, qualitySelector.value);
        });
        return item;
    }
    updateSelection(sourceId, source, selected) {
        if (selected) {
    ...
    updateQualitySelection(sourceId, quality) {
    ...
    selectAll() {
        this.currentSources.forEach((source, index) => {
            const sourceId = `source_${index}`;
            const checkbox = this.shadowRoot?.getElementById(sourceId);
            if (checkbox) {
    ...
    selectNone() {
        ...
        const checkboxes = this.shadowRoot?.querySelectorAll('.source-checkbox');
    ...
    getDownloadOptions() {
        const filenamePattern = this.shadowRoot?.getElementById('filenamePattern')?.value || '{title}.{ext}';
        const concurrentDownloads = parseInt(this.shadowRoot?.getElementById('concurrentDownloads')?.value || '2');
        const autoRetry = this.shadowRoot?.getElementById('autoRetry')?.value === 'true';
        return {
            filenamePattern,
            concurrentDownloads,
            autoRetry
