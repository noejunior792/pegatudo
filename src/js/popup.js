// PegaTudo - Enhanced Popup Logic
// Improved UI with multi-selection, batch downloads, and better UX
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function getTotalMediaCount() {
    return Object.values(currentMediaCategories).reduce(
  ...
  function renderCategorizedMedia() {
    ...
    // Render categories in order of importance
  ...
  function downloadSelectedMedia() {
    const selectedMedia = getAllMediaItems().filter(item => 
    ...
    // Separate streams from regular downloads
    const regularDownloads = selectedMedia.filter(item => 
      item.type !== 'hls' && item.type !== 'dash'
    );
    const streamDownloads = selectedMedia.filter(item => 
      item.type === 'hls' || item.type === 'dash'
    );

    showToast(`Iniciando download de ${selectedMedia.length} arquivos...`);

    // Download regular files in batch
    ...
    // Download streams individually
    streamDownloads.forEach((item, index) => {
      setTimeout(() => {
        downloadSingleItem(item, true);
      }, index * 1000); // Stagger stream downloads
    });
  }
  ...
  function startAutoRefresh() {
    ...
    // Refresh every 3 seconds when popup is open
